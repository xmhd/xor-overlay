...
# PRE-MAKE INSTALL SECTION:
    ...
    # Copy over the info pages.
    # We disabled their generation earlier, but the build system only expects to install out of the build dir, not the source.  #464008
    mkdir -p gcc/doc
    local x=
	...
	# This is because bsd userland needs fixedincludes to build gcc, while linux does not.  Both can dispose of them afterwards.
	while read x; do
		grep -q 'It has been auto-edited by fixincludes from' "${x}" && echo "Removing auto-generated header: $x" && rm -f "${x}"
...
# POST MAKE INSTALL SECTION:
	...
	# toolchain.eclass does not check ! is is_crosscompile
	[[ ! is_crosscompile ]] && \
