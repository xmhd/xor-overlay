...
linux-mod_pkg_postinst() {
	...
	if has_version sys-kernel/debian-sources[build-kernel]; then

		local INITRAMFS_ARGS

		if has_version sys-kernel/debian-sources[dracut]; then

			ewarn ">>> Dracut: building initramfs"

			dracut_modules_add="base"
			dracut_modules_skip="bootchart convertfs qemu"
			...
			INITRAMFS_ARGS+=(
				...
				--omit="${dracut_modules_skip[@]}"
			)

			if has_version sys-kernel/debian-sources[btrfs]; then
				INITRAMFS_ARGS+=( --add=btrfs )
			else
				INITRAMFS_ARGS+=( --omit=btrfs )
			fi

			if has_version sys-kernel/debian-sources[debug]; then
				INITRAMFS_ARGS+=( --stdlog=5 )
			else
				INITRAMFS_ARGS+=( --stdlog=1 )
			fi

			if has_version sys-kernel/debian-sources[firmware]; then
				INITRAMFS_ARGS+=( --fwdir=/lib/firmware )
			fi

			if has_version sys-kernel/debian-sources[luks];
				INITRAMFS_ARGS+=( --add=crypt )
			else
				INITRAMFS_ARGS+=( --omit=crypt )
			fi

			if has_version sys-kernel/debian-sources[lvm];
				INITRAMFS_ARGS+=( --add=lvm --lvmconf )
			else
				INITRAMFS_ARGS+=( --omit=lvm --nolvmconf )
			fi

			if has_version sys-kernel/debian-sources[mdadm];
				INITRAMFS_ARGS+=( --add=mdraid --mdadmconf )
			else
				INITRAMFS_ARGS+=( --omit=mdraid --nomdadmconf )
			fi

			if has_version sys-kernel/debian-sources[microcode];
				INITRAMFS_ARGS+=( --early-microcode )
			else
				INITRAMFS_ARGS+=( --no-early-microcode )
			fi

			if has_version sys-kernel/debian-sources[plymouth];
				INITRAMFS_ARGS+=( --add=plymouth )
			else
				INITRAMFS_ARGS+=( --omit=plymouth )
			fi

			if has_version sys-kernel/debian-sources[selinux];
				INITRAMFS_ARGS+=( --add=selinux )
			else
				INITRAMFS_ARGS+=( --omit=selinux )
			fi

			if has_version sys-kernel/debian-sources[systemd]; then
				INITRAMFS_ARGS+=( --add=systemd )
			else
				INITRAMFS_ARGS+=( --omit=systemd )
			fi

			if has_version sys-kernel/debian-sources[udev];
				INITRAMFS_ARGS+=( --add=udev-rules )
			else
				INITRAMFS_ARGS+=( --omit=udev-rules )
			fi

			if has_version sys-kernel/debian-sources[zfs];
				INITRAMFS_ARGS+=( --add=zfs )
			else
				INITRAMFS_ARGS+=( --omit=zfs )
			fi

			dracut ${INITRAMFS_ARGS[@]} "{ROOT}"/boot/initramfs-${KERNEL_FULL_VERSION}.img ${KERNEL_FULL_VERSION} || die ">>> Dracut: building initramfs failed"
		else
			...
			if has_version sys-kernel/debian-sources[btrfs]; then
				INITRAMFS_ARGS+=( --btrfs )
			else
				INITRAMFS_ARGS+=( --no-btrfs )
			fi

			if has_version sys-kernel/debian-sources[debug]; then
				INITRAMFS_ARGS+=( --loglevel=5 )
			else
				INITRAMFS_ARGS+=( --loglevel=1 )
			fi

			if has_version sys-kernel/debian-sources[firmware]; then
				INITRAMFS_ARGS+=( --firmware )
			else
				INITRAMFS_ARGS+=( --no-firmware )
			fi

			if has_version sys-kernel/debian-sources[luks]; then
				INITRAMFS_ARGS+=( --luks )
			else
				INITRAMFS_ARGS+=( --no-luks )
			fi

			if has_version sys-kernel/debian-sources[lvm]; then
				INITRAMFS_ARGS+=( --lvm )
			else
				INITRAMFS_ARGS+=( --no-lvm )
			fi

			if has_version sys-kernel/debian-sources[mdadm]; then
				INITRAMFS_ARGS+=( --mdadm --mdadm-config=/etc/mdadm.conf )
			else
				INITRAMFS_ARGS+=( --no-mdadm )
			fi

			if has_version sys-kernel/debian-sources[udev]; then
				INITRAMFS_ARGS+=( --udev-rules )
			else
				INITRAMFS_ARGS+=( --no-udev-rules )
			fi

			if has_version sys-kernel/debian-sources[zfs]; then
				INITRAMFS_ARGS+=( --zfs )
			else
				INITRAMFS_ARGS+=( --no-zfs )
			fi

			genkernel ${INITRAMFS_ARGS[@]} initramfs || die "Failed to build initramfs"
		fi
	fi
}
