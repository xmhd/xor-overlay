...
src_unpack() {
        default
        mv "${WORKDIR}"/JetBrains.Rider-* "${S}" || die "Failed to move/rename source dir"
}
...
src_install() {
	...
	make_desktop_entry "${MY_PN}" "Rider" "${MY_PN}" "Development;IDE;"

	# recommended by: https://confluence.jetbrains.com/display/IDEADEV/Inotify+Watches+Limit
	mkdir -p "${D}/etc/sysctl.d/" || die
	echo "fs.inotify.max_user_watches = 524288" > "${D}/etc/sysctl.d/30-rider-inotify-watches.conf" || die
}
